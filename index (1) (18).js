...
async function processNextHit() {
    ...
    try {
        ...
        while (akunDicoba < accounts.length) {
            ...
            // Deklarasi context dan page di luar blok try agar bisa dibersihkan di finally
            let context = null;
            let page = null;
            ...
            try {
                ...
                while (shouldContinue) {
                    ...
                    if (!link) {
                        // 🌟 LOGIKA AUTO SWITCH: Limit habis / tidak ada link, lanjut ke akun selanjutnya
                        shouldContinue = false; 
                        break;
                    }
                ...
                // 🌟 FORWARD ERROR KE OWNER
                await notifyOwnerError(ctx, `Scraping Akun Kxntu: ${account.username}`, error);
            } finally {
                // 🌟 STABILITAS: Pastikan memory browser selalu dibersihkan
        ...
        if (allResults.length > 0) {
            ...
            // JIKA DAPET 0 AKUN, TAMPILKAN OPSI RETRY
        ...
        // 🌟 FORWARD ERROR FATAL KE OWNER
        await notifyOwnerError(ctx, "Sistem processNextHit (Global)", error);
        await ctx.reply(`❌ *Terjadi Kesalahan Kritis:*\n${error.message}`, { parse_mode: 'Markdown' });
    } finally {
        // APAPUN YANG TERJADI (BERHASIL/GAGAL), BOT HARUS LANJUT KE ORANG BERIKUTNYA DI ANTREAN
        isProcessingHit = false;
        processNextHit(); 
    }
}
...
// --- CALLBACK UNTUK AUTO-RETRY SISA TARGET ---
bot.action(/retry_hit_(\d+)/, async (ctx) => {
    ...
    // Hapus tombol retry dari pesan agar tidak dispam user
    await ctx.editMessageReplyMarkup({
        inline_keyboard: []
    }).catch(()=>{});

    // Masukkan kembali sisa target ke antrean user
    hitQueue.push({
        ctx: ctx,
    ...
    // Jika antrean sedang sibuk
    if (isProcessingHit) {
        await ctx.reply(`⏳ <b>RETRY MASUK ANTREAN!</b>\nAntrian kamu ke: <b>${queuePosition}</b>\n<i>Mohon tunggu, target sisa kamu akan diproses otomatis...</i>`, { parse_mode: 'HTML' });
    } else {
        processNextHit();
    }
});
