    ...
    try {
        ...
        while (akunDicoba < accounts.length) {
            ...
            while (triedAccountsCount < accounts.length) {
                activeAccount = accounts[currentAccountIndex];
                
                try {
                    await page.goto('https://kxntu.com/login', { waitUntil: 'networkidle', timeout: 30000 });
                    await page.fill('#l-username', activeAccount.username);
                    await page.fill('#l-password', activeAccount.password); 
                    await page.click('button[type="submit"].btn-primary');
                    await page.waitForLoadState('networkidle');

                    const currentUrl = page.url();
                    if (currentUrl.includes('/login') || await page.$('.alert-danger, .error-message')) {
                        console.log(`⚠️ Akun ${activeAccount.username} limit / gagal login, beralih ke akun berikutnya...`);
                    ...
                    successLogin = true;
                    globalAccountIndex = (currentAccountIndex + 1) % accounts.length; 
                    break;
                } catch (err) {
                    console.log(`⚠️ Error pada akun ${activeAccount.username}: ${err.message}, mencoba akun lain...`);
                    currentAccountIndex = (currentAccountIndex + 1) % accounts.length;
                    triedAccountsCount++;
                }
            }
            ...
            while (shouldContinue) {
                ...
                const layoutScraping = `<b>[ AGASTRA ENTERPRISE - EXTRACTION ENGINE ]</b>\n` +
                                       ...
                                       `<blockquote>` +
                                       `<b>Target Account :</b> <code>${activeAccount.username}</code>\n` +
                                       `<b>Execution Flow :</b> <code>Node ${akunDicoba} of ${accounts.length}</code>\n` +
...
bot.on('document', async (ctx) => {
    ...
    try {
        ...
        let formattedLiveCookiesForMerge = []; 
...
bot.action('mode_link_to_link', async (ctx) => {
    activeConvertModes.set(ctx.from.id, 'LINK_TO_LINK'); 
    ctx.answerCbQuery();
...
bot.action('mode_link_to_cookies', async (ctx) => {
    activeConvertModes.set(ctx.from.id, 'LINK_TO_COOKIES'); 
    ctx.answerCbQuery();
...
bot.action('mode_cookies_to_link', async (ctx) => {
    activeConvertModes.set(ctx.from.id, 'COOKIES_TO_LINK'); 
    ctx.answerCbQuery();
