        ...
        while (akunDicoba < accounts.length) {
            ...
            try {
                ...
                while (shouldContinue) {
                    ...
                    // 👉 TAMBAHAN BARU: EKSTRAK LINK LANGSUNG SAAT ITU JUGA
                    const activeCookie = await resolveLinkToCookie(link);
                    if (!activeCookie) {
                        // Jika token gagal diekstrak (dead/diblokir), abaikan dan klik generate link baru
                        await page.waitForTimeout(1000);
                        continue;
                    }
                    ...
                    allResults.push({ 
                        ...
                        link: link, 
                        cookie: activeCookie, // 👉 SIMPAN COOKIE AKTIFNYA
                        country: hitCountry, 
                        plan: hitPlan 
                    });
            ...
            await ctx.replyWithDocument(
                ...
                { 
                    caption: `<b>[ AGASTRA ENTERPRISE - RESULT SUMMARY ]</b>\n` +
                             ...
                             `</blockquote>\n\n` +
                             `English 🇬🇧 :\n` +
                             `<i>Extraction is done automatically! The file above contains raw ACTIVE cookies ready to use.</i>\n\n` +
                             `Indonesian 🇮🇩 :\n` +
                             `<i>Ekstraksi dilakukan secara otomatis! File di atas hanya berisi raw cookies AKTIF yang siap digunakan.</i>`,
                    parse_mode: 'HTML',
