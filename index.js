...
async function checkPartnerGen(page) {
    try {
        // Cek apakah ada teks yang mengandung kata 'PARTNER' di halaman generator
        const pageContent = await page.content();
        if (pageContent.includes('PARTNER') || pageContent.includes('Included in your')) {
            return true;
        }

        const planElements = await page.$$('.kx-kv');
        for (const element of planElements) {
            const label = await element.$eval('.k', el => el.textContent.trim()).catch(() => '');
            if (label === 'Plan') {
        ...
        while (akunDicoba < accounts.length) {
            ...
            try {
                ...
                while (shouldContinue) {
                    ...
                    if (!link) {
                        // Jika tidak ada link, lanjut ke akun selanjutnya
                        shouldContinue = false; 
                        break;
                    }

                    // 🌟 TAMBAHKAN / PASTIKAN PENGECEKAN PARTNER INI
                    const hasPartner = await checkPartnerGen(page);
                    if (hasPartner) {
                        // Jika terdeteksi teks partner, skip link ini dan lanjut iterasi berikutnya
