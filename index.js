...
bot.command('hit', async (ctx) => {
    // 1. Verifikasi Akses: Boleh Owner ATAU VIP
    ...
    if (!isOwner && isVip) {
        if (targetTotal === 0 || targetTotal > 30) {
            targetTotal = 30;
            ctx.reply('⚠️ *INFO VIP:*\nPermintaan disesuaikan menjadi maksimal *30 link* per eksekusi untuk menjaga performa bot.', { parse_mode: 'Markdown' });
        }
    }

    // 2. MASUKKAN USER KE DALAM ANTREAN
    hitQueue.push({
        ctx: ctx,
    ...
    // 3. CEK APAKAH BOT SEDANG SIBUK?
    if (isProcessingHit) {
        // Jika sibuk, suruh nunggu di antrean
        return ctx.reply(`⏳ <b>SEDANG ADA YANG HIT NETFLIX!</b>\n\nAntrian kamu ke: <b>${queuePosition}</b>\n<i>Mohon tunggu sampai pengguna sebelumnya selesai, proses kamu akan berjalan otomatis.</i>`, { parse_mode: 'HTML' });
    } else {
        // Jika nganggur, langsung gas!
        processNextHit();
    }
});
...
async function processNextHit() {
    // Cek apakah antrean kosong? Jika kosong, istirahat.
    if (hitQueue.length === 0) {
        isProcessingHit = false;
        return;
    }

    // Tandai bot sedang sibuk memproses hit
    isProcessingHit = true;
    
    // Ambil orang urutan pertama dari antrean
    const currentTask = hitQueue.shift();
    const { ctx, targetTotal } = currentTask;

    try {
        // Beri tahu user bahwa gilirannya sudah tiba
        ...
        if (accounts.length === 0) {
            ...
            isProcessingHit = false;
            processNextHit(); // Lanjut ke orang berikutnya
            return;
        }

        // Tampilan Premium Awal (Initializing)
        // Tampilan Professional Initializing Layout
        const layoutInit = `<b>[ AGASTRA ENTERPRISE - INITIALIZATION CORE ]</b>\n` +
                           `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        ...
        while (akunDicoba < accounts.length) {
            ...
            // Tampilan Professional Login Layout
            const layoutLogin = `<b>[ AGASTRA ENTERPRISE - AUTHENTICATION MODULE ]</b>\n` +
                                `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                                ...
                                `<b>Target Quota   :</b> <code>${allResults.length} / ${targetTotal === 0 ? 'Unlimited' : targetTotal} Valid Accoun</code>\n` +
                                `<b>Session State  :</b> <code>Pending Authorization</code>\n` +
                                `</blockquote>\n\n` +
                                `<i>Status: Authenticating credentials and submitting login payload...</i>`;
            await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, layoutLogin, { parse_mode: 'HTML' });
            
            const context = await globalBrowser.newContext();
            const page = await context.newPage();
            let shouldContinue = true;

            let currentAccountIndex = globalAccountIndex;
            let triedAccountsCount = 0;
            let successLogin = false;

            while (triedAccountsCount < accounts.length) {
                ...
                try {
                    ...
                    // Cek jika akun limit harian / gagal login / error
                    const currentUrl = page.url();
                    if (currentUrl.includes('/login') || await page.$('.alert-danger, .error-message')) {
                        console.log(`⚠️ Akun ${account.username} limit / gagal login, beralih ke akun berikutnya...`);
                    ...
                    successLogin = true;
                    globalAccountIndex = currentAccountIndex; 
                    break;
                } catch (err) {
                    console.log(`⚠️ Error pada akun ${account.username}: ${err.message}, mencoba akun lain...`);
                    currentAccountIndex = (currentAccountIndex + 1) % accounts.length;
                    triedAccountsCount++;
                }
            }
            ...
            if (!successLogin) {
                await ctx.reply("❌ *Semua akun di account.txt gagal digunakan atau limit harian habis!*", { parse_mode: 'Markdown' });
                await page.close();
                await context.close();
                isProcessingHit = false;
                processNextHit();
            ...
            await page.goto('https://kxntu.com/generar', { waitUntil: 'networkidle' });
                ...
                while (shouldContinue) {
                    ...
                    // Tampilan Professional Scraping Layout
                    const layoutScraping = `<b>[ AGASTRA ENTERPRISE - EXTRACTION ENGINE ]</b>\n` +
                                           `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                    ...
                    await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, layoutScraping, { parse_mode: 'HTML' });

           // FITUR AUTO-RETRY SAAT GAGAL AMBIL LINK
            let maxRetries = 3;
            let currentRetry = 0;
            let link = null;
            ...
            while (currentRetry < maxRetries) {
                try {
                    await page.click('#btn-link', { timeout: 10000 });
                    link = await getLinkFromCodeboxGen(page);
                    if (link) break;
                } catch (e) {
                    currentRetry++;
            ...
            if (!link) {
                ...
                break;
                    ...
                    const isDuplicate = allResults.some(item => item.link === link);
                    if (isDuplicate) {
                        await page.waitForTimeout(2000); 
                        continue; 
                    }
...
// Ambil plan dan negara langsung dari elemen web kxntu.com/generar secara presisi
        ...
        // Tampilan Professional Task Finished Layout
        const resultText = `<b>[ AGASTRA ENTERPRISE - TASK EXECUTION COMPLETED ]</b>\n` +
                           `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        ...
        await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, resultText, { parse_mode: 'HTML' });
        

        if (allResults.length > 0) {
        // 🌟 HITUNG JUMLAH TIAP NEGARA (TANPA BENDERA PUTIH JIKA TIDAK TERDAFTAR)
            const countryCounts = {};
            ...
            await ctx.replyWithDocument(
                ...
                { 
                    caption: `<b>[ AGASTRA ENTERPRISE - RESULT SUMMARY ]</b>\n` +
                             `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        ...
        // FITUR AUTO-FORWARD DETAIL ERROR KE OWNER
        const errorDetail = `🚨 <b>CRITICAL ERROR DETECTED IN /hit</b>\n\n` +
                            `👤 <b>User Terdampak:</b> @${ctx.from.username || ctx.from.first_name} (<code>${ctx.from.id}</code>)\n` +
                            `📝 <b>Pesan Error:</b> <code>${error.message}</code>\n` +
                            `🛠️ <b>Stack Trace:</b>\n<pre>${error.stack ? error.stack.substring(0, 500) : 'Tidak ada detail stack'}</pre>\n\n` +
                            `📌 <i>Silakan cek terminal atau server backend Anda segera.</i>`;
        
        try {
            await bot.telegram.sendMessage(OWNER_ID, errorDetail, { parse_mode: 'HTML' });
        } catch (errForward) {
            console.error("Gagal mengirim forward error ke owner:", errForward.message);
        }

        await ctx.reply(`❌ *Terjadi Kesalahan Kritis pada Sistem:* \nPerintah gagal diproses dan laporan error telah dikirimkan ke Owner.`, { parse_mode: 'Markdown' });
    } finally {
        isProcessingHit = false;
        processNextHit(); 
    }
...
bot.action('btn_leaderboard', async (ctx) => {
    ...
    let textLeaderboard = `<b>🏆 TOP 10 RANKING CONVERTER AGASTRA STORE</b>\n─────────────────────────────\n`;
