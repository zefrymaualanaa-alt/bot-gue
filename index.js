...
bot.command('hit', async (ctx) => {
    ...
    // 🌟 PROTEKSI DOUBLE HIT: Cek apakah user sedang mengantri atau sedang diproses
...
async function processNextHit() {
    if (hitQueue.length === 0) {
        ...
        currentTaskDetail = null; // Kosongkan pelacak
        return;
    }

    isProcessingHit = true;
    
    // Ambil orang urutan pertama
    ...
    try {
        ...
        while (akunDicoba < accounts.length) {
            ...
            try {
                ...
                while (shouldContinue) {
                    ...
                    if (hasPartner) {
                        await page.waitForTimeout(1000); // SPEED-UP (Dari 2000 ke 1000)
                        continue; 
                    }
                    ...
                    allResults.push({ username: account.username, link: link, country: hitCountry, plan: hitPlan });
                    
                    // UPDATE TRACKER UNTUK /cekantrian
                    currentTaskDetail.currentCount = allResults.length;

                    await page.waitForTimeout(1000); // SPEED-UP (Dari 3000 ke 1000: Bikin scraping ngebut)
                } 
            ...
            if (akunDicoba < accounts.length && (targetTotal === 0 || allResults.length < targetTotal)) {
                await new Promise(resolve => setTimeout(resolve, 1000)); // SPEED-UP jeda pindah akun (Dari 3000 ke 1000)
            }
        } 
        ...
        if (allResults.length > 0) {
            ...
            if (sisaTarget > 0) inlineButtons.push([Markup.button.callback(`🔄 Auto-Retry Sisa Target (${sisaTarget} Link)`, `retry_hit_${sisaTarget}`)]);
        ...
        isProcessingHit = false;
        currentTaskDetail = null; // 🌟 KOSONGKAN TRACKER SETELAH SELESAI
        processNextHit(); 
    }
}
