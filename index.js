...
const userStates = {}; // Menyimpan status input user
...
function generateWelcomeText(ctx) {
    ...
    return `<b>[ AGASTRA NETFLIX HIT ]</b>\n\n` +
        ...
        `☠️ Pengguna Aktif: <code>${totalUsers} User</code>\n` +
...
bot.start(async (ctx) => {
    ...
    ctx.reply(welcomeText, { 
        ...
        ...Markup.inlineKeyboard([
            [
                Markup.button.callback('🚀 RUN SYSTEM', 'action_run_bot'), 
                Markup.button.callback('🔋 CEK ANTRIAN', 'action_cek_antrian')
            ],
            [
                Markup.button.callback('📋 MENU FITUR', 'action_menu_bot')
            ]
        ])
    });
});
...
bot.action('action_menu_bot', (ctx) => {
    ...
    const menuText = `<b>[ PUSAT KENDALI SISTEM ]</b>\n━━━━━━━━━━━━━━━━━━━━━\n\n` +
        `Berikut adalah modul perintah utama yang tersedia di sistem Agastra:\n\n` +
        `<b>🚀 Modul Eksekusi</b>\n` +
    ...
    ctx.editMessageText(menuText, {
        ...
        ...Markup.inlineKeyboard([
            ...
            [
                Markup.button.callback('⬅️ KEMBALI KE BERANDA', 'action_back_home')
            ]
        ])
    }).catch(() => {});
});
...
[Result] Terkumpul: ${allResults.length} / ${targetTotal} Sesi</pre>`;
        ...
        if (allResults.length > 0) {
            ...
            const summaryLayout = 
                ...
                `Sistem telah menyelesaikan tugas dengan sukses. Jika Anda membutuhkan standarisasi format atau verifikasi live/dead lanjutan, silakan teruskan file output di bawah ini ke bot manajemen @netflixcookieess_bot agar diproses lebih lanjut.`;
...
async function startBotWithRetry() {
    try { await bot.launch({ polling: { timeout: 30 } }); console.log('✅ Sistem Agastra Online & Beroperasi (Mode Terenkripsi)'); } 
